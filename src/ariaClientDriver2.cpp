  ...
  myHandleRangeDataCB(this, &AriaClientDriver::handleRangeData)
{
   ...
   myTwistSubscribe = myRosNodeHandle.subscribe("Twist", 1, &AriaClientDriver::topicCallBack2,this);
   myNavdataPublish = myRosNodeHandle.advertise<ariaClientDriver::AriaNavData>("AriaNavData", 1000);
   myLaserPublish = myRosNodeHandle.advertise<sensor_msgs::LaserScan>("LaserData", 50);
   myOdomPublish = myRosNodeHandle.advertise<nav_msgs::Odometry>("Odometry", 50);

   // Critical thread performing sensor data acquisition
     ArNetPacket sensorName;
     sensorName.strToBuf("lms2xx_1"); //lms2xx_1 for actual robot sim_lms2xx_1 for sim robot
...
void AriaClientDriver::handleOutput(ArNetPacket *packet)
{
  ...
  myTfBroadcaster.sendTransform(tf::StampedTransform(tf::Transform(tf::createQuaternionFromRPY(0.0,0.0,myTh/180*M_PI), tf::Vector3(myX/1000.0, myY/1000.0, 0)),current_time,"odom","base_link"));
