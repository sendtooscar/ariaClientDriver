 ...
 * v1.3 - raw laser data access
 * v1.5 beta-TODO fast odom data access from arnl server 
 */
  ...
  myHandleRangeDataCB(this, &AriaClientDriver::handleRangeData)
{
   ...
   // Critical thread performing sensor data acquisition
     ...
     myClient->request("LaserRequest", 10);
     myClient->addHandler("update", &myHandleOutputCB);
     myClient->request("update", 10);
...
void AriaClientDriver::topicCallBack2(const geometry_msgs::Twist &msg)
{
	...
	myRotRatio		=msg.angular.z*100.0;
	myLatRatio		=msg.linear.y*100.0;
	myMaxVel		=100.0;
}
...
void AriaClientDriver::handleRangeData(ArNetPacket *packet)
{
   ...
   for (int i=0; i<NumberOfReadings; i++){
    ...
    //edit to encode transmitting massege
      ...
      //msg.time_increment=0.00000;//0.01333/180;   // time between measurements [seconds] ()75Hz motor
      //msg.scan_time=0.00;//0.3;        		// time between scans [seconds]
